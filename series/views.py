...
@render_with("series/servicios.html")
def servicios(request):
    oDestacada = Serie.objects.filter(min_cover__icontains="http://").order_by("-rating")[:1]
    api = twitter.Api();
    oTwitter = api.GetUserTimeline("@django_es")[:5]
    return {"oSeries": oDestacada, "oTwitter": oTwitter}
